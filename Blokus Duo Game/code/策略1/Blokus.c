...
int second_put_test(int x,int y,char *coordinate,char color){
...
int main(int argc ,const char *argv[]){  /*   0    argv[1] "Blue"or"Red"   2 board_File  3 left_File  4 step_File*/ 
	...
	int truely_put[7];/* 1:x 2:y 3:block 4:block_area 5:turn rotate 6:turn reverse 7:block_enemy */
	truely_put[6]=-1;
	int rotate_test=0;
	int reverse_test=0;
	char enemy_color;
			...
			for(m=0;m<4;m++){  /* in this loop,it will rotate block 4 times(write in the underest loop)*/
				...
				for(i=0;i<edge_number;i++){
					if(left[left_length-1-1-2*(21-k)]=='1'){  /*read the left_pattern file and find the non-used block*/ 
						...
						for(o=0;o<size[k-1];o++){  /*choose the first location un the block*/
